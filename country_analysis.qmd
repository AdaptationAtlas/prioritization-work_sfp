...
library(ggplot2)
library(biscale)
library(ggrepel) # Only needed for plot in appendix. Remove if not needed

# Subset initial prioritization countries fron Oscar?
# TODO: Remove - this step happens earlier in the pipeline, so no longer needed
COUNTRY_SUBSET <- TRUE a0_sf_simple <- read_sf("data/a0_vect_vsimple.topojson")
...
prettyprint_countries <- function(iso3c) {
...
These countries face high levels of vulnerability and have a moderate production
...
```{r}
# --- AOW 2 ---
aow2_shocks_idx <- read_parquet("data/results/aow2_shocks.parquet") # high value = more shocks
aow2_resilience_idx <- read_parquet("data/results/aow2_resilience.parquet") # high value = more resilience
...
```{r}
# --- AOW 3 ---
aow3_constraints_idx <- read_parquet("data/results/aow3_constraints.parquet") # high value = more constraints
aow3_capacity_idx <- read_parquet("data/results/aow3_capacity.parquet") # high value = more capacity
...
grape_df <- biscale::bi_class(
  grape_df,
  hr_per_rpop, # Human Resources per rural population
  rd_pct_gdp, # R&D as a share of Ag GDP
  style = "quantile",
  dim = 3
)
```
